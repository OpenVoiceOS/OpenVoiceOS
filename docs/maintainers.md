...
**`lists/deprecated.list`** holds packages that have been removed from OVOS tracking. The `make_alpha_testing.py` script appends here automatically when a package disappears from alpha.

**`lists/unstable.list`** holds packages explicitly excluded from `constraints-testing.txt` and `constraints-stable.txt`. This file is auto-generated by `make_alpha_testing.py` based on dependency conflict detection. Do not edit it manually.

### Step-by-Step Release Workflow

#### Prerequisites

- [`uv`](https://github.com/astral-sh/uv) must be installed (`pip install uv` or through the official installer).
...
This is the smarter, incremental approach. It reads `constraints-alpha.txt` and `constraints-testing.txt` and produces an updated `constraints-testing.txt` by doing the following:

- **Deprecating** packages that disappeared from alpha: it appends them to `lists/deprecated.list` and drops them from testing.
...
- **Preserving** existing lower bounds from testing to avoid regression, while updating upper bounds based on the new alpha version.
...
> **Through GitHub Actions:** The [Sync Testing Constraints workflow](.github/workflows/make_alpha_testing.yml) can be triggered manually from the Actions tab. It installs Python 3.10 through 3.14 with `uv`, creates the required venvs, runs the script, runs the conflict report, and commits `constraints-testing.txt`, `lists/unstable.list`, `lists/deprecated.list`, and `docs/conflicts.md` in one step.
...
This script creates isolated `uv` virtual environments for each supported Python version (3.10 to 3.15) under `.venvs/` and does a dry-run install to check for resolution conflicts. It prints `PASS`, `FAIL`, or `SKIP` for each version and exits non-zero if any version fails.

> Python versions that are not installed locally are automatically skipped.

#### Step 4: Commit

Only the generated constraints files and any changes to `lists/` need to be committed. Do not commit `resolved-constraints.txt` or `.venvs/`.
...
2. Add the package name, optionally with a minimum version hint:
   ...
   ```
3. Run the full workflow (Steps 1 through 3 above) to regenerate constraints files.

### Removing or Deprecating a Package

1. Delete the entry from the relevant `.list` file.
2. Run `make_alpha_testing.py`. It automatically detects the removal and appends the package to `lists/deprecated.list`.
...
| **Python version restrictions** | Packages present in testing but restricted to older Pythons through a `; python_version < "X.Y"` marker because a transitive dependency lacks wheels for newer versions. |
...
|---|---|---|---|
| Alpha | `constraints-alpha.txt` | `pkg>=X.Y.Z` | Minimum only. The `--pre` flag is required at install time |
...
---
[← Distro packaging](distro-packaging.md) · [Home](README.md)
